...
export function installOrbit(canvas,app,{touchRotate=true}={}){
 ...
 canvas.addEventListener('pointerdown',event=>{
  ...
  app.gallery_dragging(touchRotate||event.pointerType!=='touch'||pointers.size>1);
 });
 canvas.addEventListener('pointermove',event=>{
  const previous=pointers.get(event.pointerId);if(!previous)return;
  const before=[...pointers.values()];pointers.set(event.pointerId,[event.clientX,event.clientY]);
  if(pointers.size===1){
   if(!touchRotate&&event.pointerType==='touch')return;
 ...
 for(const type of ['pointerup','pointercancel','lostpointercapture'])canvas.addEventListener(type,event=>{pointers.delete(event.pointerId);app.gallery_dragging(pointers.size>(!touchRotate&&event.pointerType==='touch'?1:0));});
