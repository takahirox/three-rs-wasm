...
if (!entry || entry.status === 'excluded' || !entry.port) {
 ...
 try {
  ...
  const runtimeRevision = 'helpers-formats-1';
  ...
  if (closing) { app.free(); app = null; } else {
   ...
   if ([75,134,230].includes(entry.port.example)) window.addEventListener('pointermove',event=>app.gallery_pointer(event.clientX/canvas.clientWidth*2-1,1-event.clientY/canvas.clientHeight*2));
   ...
   if ([214,215,217,218,225,231,233].includes(entry.port.example)) {
   ...
   if ([193,210,214,215,217,218,225,231,233].includes(entry.port.example)) for(const [type,down] of [['keydown',true],['keyup',false]])document.addEventListener(type,event=>app.gallery_key(event.keyCode,down));
