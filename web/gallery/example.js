...
if (!entry || entry.status === 'excluded' || !entry.port) {
 ...
 try {
  ...
  const runtimeRevision = 'environment-materials-1';
  ...
  if (closing) { app.free(); app = null; } else {
   ...
   if ([39,50,55,56,150,151,163,164,165,174,178].includes(entry.port.example)) canvas.addEventListener('pointermove',event=>app.gallery_pointer(event.offsetX/canvas.clientWidth*2-1,1-event.offsetY/canvas.clientHeight*2));
