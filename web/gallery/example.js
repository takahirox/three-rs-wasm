...
if (!entry || entry.status === 'excluded' || !entry.port) {
 ...
 try {
  ...
  const runtimeRevision = 'terrain-loaders-1';
  ...
  if (closing) { app.free(); app = null; } else {
   ...
   if ([214,215,217,218].includes(entry.port.example)) {
   ...
   if ([193,210,214,215,217,218].includes(entry.port.example)) for(const [type,down] of [['keydown',true],['keyup',false]])document.addEventListener(type,event=>app.gallery_key(event.keyCode,down));
