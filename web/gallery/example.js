...
if (!entry || entry.status === 'excluded' || !entry.port) {
 ...
 try {
  ...
  const runtimeRevision = 'trackball-sprites-1';
  ...
  if (closing) { app.free(); app = null; } else {
   ...
   if ([75,134].includes(entry.port.example)) window.addEventListener('pointermove',event=>app.gallery_pointer(event.clientX/canvas.clientWidth*2-1,1-event.clientY/canvas.clientHeight*2));
   if ([39,50,55,56,150,151,163,164,165,174,178,185,186,187,188,192,193,194,198,203,204,205,209].includes(entry.port.example)) canvas.addEventListener('pointermove',event=>app.gallery_pointer(event.offsetX/canvas.clientWidth*2-1,1-event.offsetY/canvas.clientHeight*2));
   ...
   if ([214,215,217].includes(entry.port.example)) {
    ...
    if(entry.port.example!==217)canvas.addEventListener('wheel',event=>{event.preventDefault();app.gallery_input(0,0,event.deltaY,false);},{passive:false});
   }
   if ([193,210,214,215,217].includes(entry.port.example)) for(const [type,down] of [['keydown',true],['keyup',false]])document.addEventListener(type,event=>app.gallery_key(event.keyCode,down));
