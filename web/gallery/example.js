...
if (!entry || entry.status === 'excluded' || !entry.port) {
 ...
 try {
  ...
  const runtimeRevision = 'stereo-loaders-1';
  ...
  if (closing) { app.free(); app = null; } else {
   ...
   if ([39,50,55,56,150,151,163,164,165,174,178,185,186,187,188,192,193,194,198,203,204,205].includes(entry.port.example)) canvas.addEventListener('pointermove',event=>app.gallery_pointer(event.offsetX/canvas.clientWidth*2-1,1-event.offsetY/canvas.clientHeight*2));
   ...
   if (entry.port.example === 193) for(const [type,down] of [['keydown',true],['keyup',false]])document.addEventListener(type,event=>app.gallery_key(event.keyCode,down));
