...
if (!entry || entry.status === 'excluded' || !entry.port) {
 ...
 try {
  ...
  const runtimeRevision = 'passes-decals-1';
  ...
  const resize = () => { canvas.width = Math.max(1, Math.round(innerWidth * ([132,141,277,282].includes(entry.port.example)?1:devicePixelRatio))); canvas.height = Math.max(1, Math.round(innerHeight * ([132,141,277,282].includes(entry.port.example)?1:devicePixelRatio))); app?.request_render(); };
  ...
  if (closing) { app.free(); app = null; } else {
   ...
   if ([214,215,217,218,225,231,233,245,250,251,255,256,264,266,269,275,281,292].includes(entry.port.example)) {
