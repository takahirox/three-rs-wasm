...
if (!entry || entry.status === 'excluded' || !entry.port) {
 ...
 try {
  ...
  const runtimeRevision = 'texture-flares-1';
  ...
  const resize = () => { canvas.width = Math.max(1, Math.round(innerWidth * ([132,141,277,282,293].includes(entry.port.example)?1:devicePixelRatio))); canvas.height = Math.max(1, Math.round(innerHeight * ([132,141,277,282,293].includes(entry.port.example)?1:devicePixelRatio))); app?.request_render(); };
  ...
  if (entry.port.example === 275) {
   // The original's hidden looping video, restarted at 3 s on play.
   ...
   for(const [name,type] of [['sintel.ogv','video/ogg; codecs="theora, vorbis"'],['sintel.mp4','video/mp4; codecs="avc1.42E01E, mp4a.40.2"']]){const source=document.createElement('source');source.src=`assets/exporters-video/${name}`;source.type=type;video.append(source);}
   video.addEventListener('play',()=>{video.currentTime=3;});
  ...
  if (closing) { app.free(); app = null; } else {
   ...
   if ([214,215,217,218,225,231,233,245,250,251,255,256,264,266,269,275,281,292,298,301].includes(entry.port.example)) {
    ...
    if(![217,218,292].includes(entry.port.example))canvas.addEventListener('wheel',event=>{event.preventDefault();app.gallery_input(0,0,event.deltaY,false);},{passive:false});
   }
   ...
   if ([193,210,214,215,217,218,225,231,233,245,251,255,264,265,269,301].includes(entry.port.example)) for(const [type,down] of [['keydown',true],['keyup',false]])document.addEventListener(type,event=>app.gallery_key(event.keyCode,down));
