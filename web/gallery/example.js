...
if (!entry || entry.status === 'excluded' || !entry.port) {
 ...
 try {
  ...
  const runtimeRevision = 'shapes-lights-1';
  ...
  if (closing) { app.free(); app = null; } else {
   ...
   if ([214,215,217,218,225,231,233,245].includes(entry.port.example)) {
   ...
   if ([193,210,214,215,217,218,225,231,233,245].includes(entry.port.example)) for(const [type,down] of [['keydown',true],['keyup',false]])document.addEventListener(type,event=>app.gallery_key(event.keyCode,down));
