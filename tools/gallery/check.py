...
for record in json.loads((ROOT/"web/gallery/assets/interactive-objects-manifest.json").read_text())+json.loads((ROOT/"web/gallery/assets/interactive-scenes-manifest.json").read_text())+json.loads((ROOT/"web/gallery/assets/views-loaders-manifest.json").read_text())+json.loads((ROOT/"web/gallery/assets/stereo-loaders-manifest.json").read_text())+json.loads((ROOT/"web/gallery/assets/controls-attributes-manifest.json").read_text())+json.loads((ROOT/"web/gallery/assets/trackball-sprites-manifest.json").read_text()):
 assert hashlib.sha256((ROOT/"web/gallery/assets"/record["file"]).read_bytes()).hexdigest()==record["sha256"]
 assert hashlib.sha256((ROOT/".cache/three-r186"/record["source"]).read_bytes()).hexdigest()==record["sha256"]
