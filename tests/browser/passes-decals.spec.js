...
test.use({deviceScaleFactor:Number(process.env.PASSES_DPR||1)});
const view='canvas';
...
const cases={
 webgl_postprocessing_backgrounds:{times:[0],parameters:[[6,0.5,0.5],[4,0.5,0.5],[5,false,0],[3,false,0],[1,'blue',2],[1,'red',4],[7,false,0],[0,false,0],[7,true,1],[0,true,1]],restore:[[0,true,1],[1,'white',1],[3,true,1],[4,1,1],[5,true,1],[6,1,1],[7,true,1]],at:0,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_shader_lava:{times:[0,1,2],parameters:[],at:1},
 webgl_ubo:{times:[0,1,2],parameters:[],at:1,antialias:true},
 webgl_postprocessing_rgb_halftone:{times:[0,1],parameters:[[0,2,1],[0,3,2],[0,4,3],[0,5,4],[0,1,0],[1,8,8],[2,30,30],[5,0.5,0.5],[6,true,1],[6,false,0],[7,0.5,0.5],[8,2,1],[8,3,2],[8,4,3],[8,5,4],[8,1,0],[9,true,1]],restore:[[1,4,4],[2,15,15],[5,0,0],[7,1,1],[9,false,0]],at:1,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_decals:{times:[0],parameters:[],script:[['move',256,240,0],['down',0,256,240,null],['up',0,256,240,0],['move',240,215,0],['down',0,240,215,null],['up',0,240,215,0],['param',2,false,0,null],['param',0,25,25,null],['down',0,272,268,null],['up',0,272,268,0],['param',3,null,1,0]],residency:[['move',256,240,0],['move',240,215,0]],at:0,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
};
...
const msaaLimits={webgl_ubo:[.015,.3]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Passes and decals official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/passes-decals.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Passes and decals GPU residency: ${kind}`,async({page},info)=>{
...
test('Passes and decals resident geometry and official draw workload',async({page},info)=>{
...
if(count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/passes-decals.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const kind of [])test(`Passes and decals on-demand scene stays idle: ${kind}`,async({page})=>{
