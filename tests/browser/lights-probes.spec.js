...
test.use({deviceScaleFactor:Number(process.env.LIGHTS_DPR||1)});
const view='canvas';
...
const cases={
 webgpu_clearcoat:{times:[0,1,2.5],parameters:[],at:2.5,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 misc_controls_fly:{times:[0,1,2],limits:[.008,.6],parameters:[],at:2,antialias:true,script:[['move',300,280,null],['wait',2.5],['key','w',true,null],['wait',3],['key','w',false,3.5],['key','q',true,null],['wait',4],['key','q',false,4.5]]},
 webgpu_shadowmap_pointlight:{times:[0,1,2.5,4],parameters:[],at:4,antialias:true,drag:[[256,256],[330,300]]},
 webgpu_lightprobe:{times:[0],parameters:[[0,0.5,0.5],[1,0.2,0.2],[2,0.3,0.3]],restore:[[0,1,1],[1,0.6,0.6],[2,1,1]],at:0,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgpu_tonemapping:{times:[0],pick:true,parameters:[[0,'None',0],[0,'Linear',1],[0,'Reinhard',2],[0,'Cineon',3],[0,'ACESFilmic',4],[0,'AgX',5],[0,'Neutral',6],[1,1.5,1.5],[2,0.8,0.8],[3,0.5,0.5]],restore:[[1,1,1],[2,0.3,0.3],[3,1,1]],at:0,antialias:true,settle:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
};
...
const msaaLimits={webgpu_clearcoat:[.02,.4],misc_controls_fly:[.02,.4],webgpu_shadowmap_pointlight:[.02,.4],webgpu_lightprobe:[.02,.4],webgpu_tonemapping:[.02,.6]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Lights and probes official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/lights-probes.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Lights and probes GPU residency: ${kind}`,async({page},info)=>{
...
test('Lights and probes resident geometry and official draw workload',async({page},info)=>{
...
if(count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/lights-probes.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  // The port draws the equirectangular tone-mapping background with a fullscreen triangle;
  // Three.js uses a 5,952-index sphere. All scene mesh draws must still match.
  const referenceDraws=kind==='webgpu_tonemapping'?pair.reference.draws.filter(d=>d.count!==5952):pair.reference.draws;
...
for(const kind of [])test(`Lights and probes on-demand scene stays idle: ${kind}`,async({page})=>{
