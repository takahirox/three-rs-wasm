...
test.use({deviceScaleFactor:Number(process.env.MATERIALS_DPR||1)});
const controls={materials_wireframe:[[0,0],[0,2],[0,4],[0,1]],materials_texture_filters:[],geometry_shapes:[],geometry_colors_lookuptable:[[0,1],[0,2],[0,3],[0,0]],buffergeometry_uint:[]};
const official=kind=>'webgl_'+kind;
for(const [kind,parameters] of Object.entries(controls))for(const samples of (kind==='materials_texture_filters'?[4]:[1,4]))for(const diagnostic of (['materials_wireframe','geometry_shapes','buffergeometry_uint'].includes(kind)&&samples===4?['','gpuGeometry']:['']))test(`Geometry materials official rendering: ${kind} samples=${samples} ${diagnostic}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  await page.setViewportSize({width:512,height:512});await page.goto(runtime==='reference'?`/reference/three-js/geometry-materials.html?id=${official(kind)}&samples=${samples}&${diagnostic}=1`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  if(['materials_wireframe','geometry_colors_lookuptable','geometry_shapes'].includes(kind)){await page.mouse.move(200,200);await page.mouse.down();await page.mouse.move(250,225,{steps:5});await page.mouse.up();if(kind==='geometry_shapes')for(let i=0;i<180;i++)await capture(2.25,null,false);await capture(2.25);await page.mouse.wheel(0,-100);await capture(2.25);}
  if(kind==='materials_texture_filters'){await page.mouse.move(296,276);for(let i=0;i<180;i++)await capture(2.25,null,false);await capture(2.25);}
 ...
 writeFileSync(info.outputPath('comparison.json'),JSON.stringify(results,null,2));expect(errors).toEqual([]);for(const r of results){const allowances={materials_wireframe:[.12,6],geometry_shapes:[.035,1],buffergeometry_uint:[.36,6.5]};const [limit,meanLimit]=samples===4&&!diagnostic?(allowances[kind]??[.005,.6]):[.005,.6];expect(r.fraction,JSON.stringify(r)).toBeLessThanOrEqual(limit);expect(r.meanError,JSON.stringify(r)).toBeLessThanOrEqual(meanLimit);}
});

for(const kind of Object.keys(controls)){const id=`${official(kind)}`;
 test(`Geometry materials GPU residency: ${kind}`,async({page},info)=>{
...
test('Geometry materials resident geometry and official draw workload',async({page},info)=>{
 ...
 await page.addInitScript(()=>{
  ...
  const setPipeline=GPURenderPassEncoder.prototype.setPipeline;GPURenderPassEncoder.prototype.setPipeline=function(p){this.scenePipeline=p.label==='material pipeline';return setPipeline.call(this,p);};
 ...
 for(const kind of Object.keys(controls)){
  const pair={kind};
  for(const runtime of ['reference','rust']){
   await page.goto(runtime==='reference'?`/reference/three-js/geometry-materials.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  expect.soft(pair.rust.attributeBuffers,kind).toEqual({"materials_wireframe":[],"materials_texture_filters":[],"geometry_shapes":[],"geometry_colors_lookuptable":[2112],"buffergeometry_uint":[24000000]}[kind]);
