...
test.use({deviceScaleFactor:Number(process.env.PARTICLES_DPR||1)});
const controls={points:[],points_interleaved:[],custom_attributes_particles:[],instancing_billboards:[],selective_draw:[[0,1],[0,1],[1,1]]};
const official=kind=>'webgl_buffergeometry_'+kind;
for(const [kind,parameters] of Object.entries(controls))for(const samples of (kind==='selective_draw'?[1,4]:[1]))for(const diagnostic of (kind==='points'||kind==='points_interleaved'?['','pointQuads']:kind==='selective_draw'&&samples===4?['','webgpuLines']:['']))test(`Buffer particles official rendering: ${kind} samples=${samples} ${diagnostic}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  await page.setViewportSize({width:512,height:512});await page.goto(runtime==='reference'?`/reference/three-js/buffer-particles.html?id=${official(kind)}&samples=${samples}&${diagnostic}=1`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
 ...
 writeFileSync(info.outputPath('comparison.json'),JSON.stringify(results,null,2));expect(errors).toEqual([]);for(const r of results){const points=kind==='points'||kind==='points_interleaved';const lineMSAA=kind==='selective_draw'&&samples===4;const limit=diagnostic ? .005 : points ? .032 : lineMSAA ? .16 : .005;const meanLimit=diagnostic ? .6 : points ? 1.2 : lineMSAA ? 3 : .6;expect(r.fraction,JSON.stringify(r)).toBeLessThanOrEqual(limit);expect(r.meanError,JSON.stringify(r)).toBeLessThanOrEqual(meanLimit);}
});

for(const kind of Object.keys(controls)){const id=`${official(kind)}`;
 test(`Buffer particles GPU residency: ${kind}`,async({page},info)=>{
...
test('Buffer particles resident geometry and official draw workload',async({page},info)=>{
 ...
 for(const kind of Object.keys(controls)){
  const pair={kind};
  for(const runtime of ['reference','rust']){
   await page.goto(runtime==='reference'?`/reference/three-js/buffer-particles.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  const bytes={points:12000000,points_interleaved:8000000,custom_attributes_particles:1200000,instancing_billboards:900000,selective_draw:80000}[kind];
  expect(pair.rust.attributeBuffers,kind).toEqual([bytes]);
...
test('Buffer particles visibility controls keep one draw and upload only changed visibility',async({page},info)=>{
 const counts={};
 await page.addInitScript(()=>{window.storageBytes=0;window.draws=[];
   const write=GPUQueue.prototype.writeBuffer;GPUQueue.prototype.writeBuffer=function(b,o,data,...args){if(b.usage&GPUBufferUsage.STORAGE)storageBytes+=data.byteLength;return write.call(this,b,o,data,...args);};
   for(const key of ['draw','drawIndexed']){const fn=GPURenderPassEncoder.prototype[key];GPURenderPassEncoder.prototype[key]=function(...a){if(a[0]>3)draws.push(a[0]);return fn.apply(this,a);};}
 });
 for(const runtime of ['reference','rust']){
  await page.goto(runtime==='reference'?'/reference/three-js/buffer-particles.html?id=webgl_buffergeometry_selective_draw':'/web/gallery/example.html?id=webgl_buffergeometry_selective_draw&still=1');
  await page.waitForFunction(()=>document.querySelector('canvas')?.dataset.ready==='true'||Number(document.querySelector('canvas')?.dataset.frames)>0);
  counts[runtime]=[];
  for(const id of ['hideLines','hideLines','showAllLines']){
   const prev=await page.locator('canvas').getAttribute('data-frames');await page.evaluate(()=>{storageBytes=0;draws=[];});
   await page.locator('#'+id).click();
   if(runtime==='reference')await page.evaluate(()=>renderFixture(1));else await page.waitForFunction(n=>document.querySelector('canvas').dataset.frames!==n,prev);
   const count=await page.evaluate(runtime=>runtime==='reference'?reference.scene.children[0].geometry.attributes.visible.array.filter(v=>v===0).length/2:Number(app.gallery_status().match(/(\d+) culled/)[1]),runtime);
   counts[runtime].push(count);
   if(runtime==='rust'){
    await expect(page.locator('#line-count')).toHaveText(`1 draw call, 20,000 lines, ${count} culled`);
    expect(await page.evaluate(()=>({storageBytes,draws}))).toEqual({storageBytes:80000,draws:[40000]});
    const prev=await page.locator('canvas').getAttribute('data-frames');await page.evaluate(()=>{storageBytes=0;draws=[];app.gallery_time(2);});await page.waitForFunction(n=>document.querySelector('canvas').dataset.frames!==n,prev);
    expect(await page.evaluate(()=>({storageBytes,draws}))).toEqual({storageBytes:0,draws:[40000]});
   }
  }
 }
 expect(counts.rust).toEqual(counts.reference);expect(counts.rust[0]).toBeGreaterThan(0);expect(counts.rust[1]).toBeGreaterThan(counts.rust[0]);expect(counts.rust[2]).toBe(0);
 writeFileSync(info.outputPath('visibility.json'),JSON.stringify(counts,null,2));
});
