...
test('Rust app renders, animates, selects objects and releases its event loop', async ({ page }) => {
  ...
  await expect(canvas).toHaveAttribute('data-delta','0');
  await page.evaluate(()=>{Object.defineProperty(document,'hidden',{configurable:true,get:()=>false});document.dispatchEvent(new Event('visibilitychange'));});
  await expect.poll(async()=>Number(await canvas.getAttribute('data-delta'))).toBeGreaterThan(0);
  expect(Number(await canvas.getAttribute('data-delta'))).toBeLessThan(1);
...
test('Rust scene rebuild updates attributes, groups and draw range and releases old resources',async({page})=>{
  ...
  await expect(canvas).toHaveAttribute('data-rebuilds','20');
