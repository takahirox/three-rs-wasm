...
test.use({deviceScaleFactor:Number(process.env.HELPERS_DPR||1)});
const view='canvas';
...
const cases={
 webgl_loader_pdb:{times:[0,.5,1.5,3],parameters:[[0,'ethanol.pdb',0,3],[0,'buckyball.pdb',15,3],[0,'ybco.pdb',14,3],[0,'caffeine.pdb',2,3]],at:3,antialias:true,script:[['drag',256,256,330,300,0,3.5],['wheel',256,256,-400,4],['drag',256,256,300,200,2,4.5],['key','KeyS',true,null],['drag',256,300,256,200,0,5],['key','KeyS',false,null],['wait',5.5]]},
 webgl_helpers:{times:[0,.7,1.5,3,5],parameters:[],at:0,antialias:false},
 webgl_modifier_simplifier:{limits:[.008,.6],times:[0],parameters:[[0,.37,.37],[0,.01,.01],[0,1,1],[0,.125,.125]],at:0,antialias:true,drag:[[256,256],[330,300]]},
 webgl_loader_amf:{times:[0],parameters:[],at:0,antialias:true,drag:[[256,256],[330,300]],pan:[[256,256],[300,220]]},
 webgl_loader_texture_tiff:{times:[0],parameters:[],at:0,antialias:true},
};
...
// WebGL/WebGPU MSAA resolve bounds, documented in docs/helpers-formats.md. The same
// scenes must also pass the ordinary threshold with MSAA disabled on both sides, except
// the flat-shaded simplified head, whose facet edges take derivative normals.
const msaaLimits={webgl_loader_pdb:[.015,.35],webgl_modifier_simplifier:[.015,.35],webgl_loader_amf:[.04,.7]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Helpers and formats official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/helpers-formats.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Helpers and formats GPU residency: ${kind}`,async({page},info)=>{
...
test('Helpers and formats resident geometry and official draw workload',async({page},info)=>{
...
if(a[0]!==4||count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/helpers-formats.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const kind of ['webgl_modifier_simplifier','webgl_loader_amf','webgl_loader_texture_tiff'])test(`Helpers and formats on-demand scene stays idle: ${kind}`,async({page})=>{
