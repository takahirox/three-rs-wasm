...
test.use({deviceScaleFactor:Number(process.env.TSL_MATERIALS_DPR||1)});
const controls={materials_sss:[[0,.5],[1,2],[2,.2],[3,8],[4,32]],materials_toon:[],skinning_instancing:[],oit:[[0,false],[1,.8],[0,true],[1,.1]],materials_envmaps_groundprojected:[[0,false],[0,true]]};
for(const [kind,parameters] of Object.entries(controls))test(`TSL materials official rendering: ${kind}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  await page.setViewportSize({width:512,height:512});await page.goto(runtime==='reference'?`/reference/three-js/tsl-materials.html?id=webgpu_${kind}`:`/web/gallery/example.html?id=webgpu_${kind}&still=1`);
  ...
  const frames=[];const settle=['oit'].includes(kind)?360:180;
...
for(const kind of Object.keys(controls)){const id=`webgpu_${kind}`;
 test(`TSL materials GPU residency: ${kind}`,async({page},info)=>{
...
test('TSL materials GPU workload and resident attributes',async({page},info)=>{
 ...
 for(const kind of Object.keys(controls)){
  const pair={kind};
  for(const runtime of ['reference','rust']){
   await page.goto(runtime==='reference'?`/reference/three-js/tsl-materials.html?id=webgpu_${kind}`:`/web/gallery/example.html?id=webgpu_${kind}&still=1`);
  ...
  // Ground's transmission path retains opaque HDR pixels rather than redrawing
  // the opaque scene; all original geometry must still occur in the Rust frame.
  if(kind==='materials_envmaps_groundprojected'){
   const unique=rows=>[...new Map(rows.map(d=>[JSON.stringify(d),d])).values()];
   expect.soft(sort(unique(pair.rust.draws)),kind).toEqual(sort(unique(pair.reference.draws)));
   expect.soft(pair.rust.draws.length,kind).toBeLessThanOrEqual(pair.reference.draws.length);
  ...
  if(kind==='skinning_instancing'){
   ...
   expect.soft(palettes.length,kind).toBe(1);
