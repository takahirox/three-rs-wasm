...
for(const entry of catalog.examples.filter(e=>e.port))test(`Rust gallery runtime: ${entry.id}`,async({page})=>{
 ...
 await expect.poll(async()=>Number(await canvas.getAttribute('data-frames')),{timeout:90000}).toBeGreaterThan([16,28,38,153,156,157,193,195,206,213,220,224,226,235,236,237,240,242,255,277,283,296].includes(entry.port.example) ? 0 : 2);
