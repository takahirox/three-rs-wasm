...
test.use({deviceScaleFactor:Number(process.env.TSL_NEXT_DPR||1)});
const controls={instance_path:[],postprocessing_sobel:[[0,false],[0,true]],postprocessing_smaa:[[0,false],[0,true],[1,false],[1,true]],postprocessing_3dlut:[[1,0],[1,.5],[1,1],...[1,2,3,4,5,6,7,8,0].map(i=>[0,i])],parallax_uv:[[0,0],[0,1],[0,.4],[1,.2],[1,.5],[2,1],[2,5]]};
for(const [kind,parameters] of Object.entries(controls))test(`TSL next official rendering: ${kind}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  await page.setViewportSize({width:512,height:512});await page.goto(runtime==='reference'?`/reference/three-js/tsl-next.html?id=webgpu_${kind}`:`/web/gallery/example.html?id=webgpu_${kind}&still=1`);
...
for(const kind of Object.keys(controls)){const id=`webgpu_${kind}`;
 test(`TSL next GPU residency: ${kind}`,async({page},info)=>{
...
test('TSL next GPU workload and resident attributes',async({page},info)=>{
 ...
 for(const kind of Object.keys(controls)){
  const pair={kind};
  for(const runtime of ['reference','rust']){
   await page.goto(runtime==='reference'?`/reference/three-js/tsl-next.html?id=webgpu_${kind}`:`/web/gallery/example.html?id=webgpu_${kind}&still=1`);
  ...
  const background=['instance_path','parallax_uv'].includes(kind);
