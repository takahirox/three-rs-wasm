...
test.use({deviceScaleFactor:Number(process.env.TSL_ENVIRONMENT_DPR||1)});
const controls={cubemap_mix:[],cubemap_adjustments:[[0,1],[0,-1],[0,0],[1,.5],[2,2],[3,3],[4,.5],[5,2],[6,1],[7,.2]],materials_envmaps_bpcem:[[0,false],[0,true],[1,0],[1,.5],[1,1]],materials_alphahash:[[0,0],[0,1],[0,.5],[1,false],[1,true],[2,0],[2,4]],postprocessing_ca:[[0,false],[0,true],[1,0],[1,3],[2,-.5],[3,.2],[4,.5],[5,false],[6,false]]};
for(const [kind,parameters] of Object.entries(controls))test(`TSL environment official rendering: ${kind}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  await page.setViewportSize({width:512,height:512});await page.goto(runtime==='reference'?`/reference/three-js/tsl-environment.html?id=webgpu_${kind}`:`/web/gallery/example.html?id=webgpu_${kind}&still=1`);
...
for(const kind of Object.keys(controls)){const id=`webgpu_${kind}`;
 test(`TSL environment GPU residency: ${kind}`,async({page},info)=>{
...
test('TSL environment GPU workload and resident attributes',async({page},info)=>{
 ...
 for(const kind of Object.keys(controls)){
  const pair={kind};
  for(const runtime of ['reference','rust']){
   await page.goto(runtime==='reference'?`/reference/three-js/tsl-environment.html?id=webgpu_${kind}`:`/web/gallery/example.html?id=webgpu_${kind}&still=1`);
