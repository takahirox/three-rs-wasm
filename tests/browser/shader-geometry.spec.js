...
test.use({deviceScaleFactor:Number(process.env.GEOMETRY_DPR||1)});
const controls={buffergeometry_attributes_none:[],buffergeometry_attributes_integer:[],buffergeometry_instancing:[[0,0],[0,12345],[0,50000]],buffergeometry_instancing_interleaved:[],materials_modified:[]};
const official=kind=>'webgl_'+kind;
for(const [kind,parameters] of Object.entries(controls))for(const samples of (kind==='buffergeometry_instancing'||kind==='buffergeometry_instancing_interleaved'?[1]:[1,4]))for(const diagnostic of (kind.startsWith('buffergeometry_attributes')&&samples===4?['','gpuGeometry']:['']))test(`Shader geometry official rendering: ${kind} samples=${samples} ${diagnostic}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  await page.setViewportSize({width:512,height:512});await page.goto(runtime==='reference'?`/reference/three-js/shader-geometry.html?id=${official(kind)}&samples=${samples}&${diagnostic}=1`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  const capture=async(t,parameter=null,save=true)=>{
   ...
   await page.evaluate(async({runtime,t,parameter})=>{if(parameter){const [i,v]=parameter;if(runtime==='reference')fixtureParameter(i,v);else app.tsl_parameter(i,Number(v));}if(runtime==='reference'){reference.camera.aspect=innerWidth/innerHeight;reference.camera.updateProjectionMatrix();reference.renderer.setSize(innerWidth,innerHeight);await renderFixture(t);}else app.gallery_time(t);},{runtime,t,parameter});
  ...
  if(kind==='materials_modified'){await page.mouse.move(200,200);await page.mouse.down();await page.mouse.move(250,225,{steps:5});await page.mouse.up();await capture(2.25);await page.mouse.wheel(0,-100);await capture(2.25);}
 ...
 writeFileSync(info.outputPath('comparison.json'),JSON.stringify(results,null,2));expect(errors).toEqual([]);for(const r of results){const nativeMsaa=samples===4&&!diagnostic;const dense=kind.startsWith('buffergeometry_attributes');const limit=nativeMsaa?(dense?.16:.006):.005,meanLimit=nativeMsaa&&dense?5:.6;expect(r.fraction,JSON.stringify(r)).toBeLessThanOrEqual(limit);expect(r.meanError,JSON.stringify(r)).toBeLessThanOrEqual(meanLimit);}
});

for(const kind of Object.keys(controls)){const id=`${official(kind)}`;
 test(`Shader geometry GPU residency: ${kind}`,async({page},info)=>{
...
test('Shader geometry resident geometry and official draw workload',async({page},info)=>{
 ...
 for(const kind of Object.keys(controls)){
  const pair={kind};
  for(const runtime of ['reference','rust']){
   await page.goto(runtime==='reference'?`/reference/three-js/shader-geometry.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
   ...
   for(let t=1;t<=3;t++){const prev=await page.locator('canvas').getAttribute('data-frames');await page.evaluate(async({runtime,t})=>{if(t===3)resetWork();if(runtime==='reference'){reference.camera.aspect=innerWidth/innerHeight;reference.camera.updateProjectionMatrix();reference.renderer.setSize(innerWidth,innerHeight);await renderFixture(t);}else app.gallery_time(t);},{runtime,t});if(runtime==='rust')await page.waitForFunction(p=>document.querySelector('canvas').dataset.frames!==p,prev);}
   pair[runtime]=await page.evaluate(()=>({...work,attributeBuffers}));
  }
  expect.soft(pair.rust.attributeBuffers,kind).toEqual({"buffergeometry_attributes_none":[],"buffergeometry_attributes_integer":[360000],"buffergeometry_instancing":[3000000],"buffergeometry_instancing_interleaved":[320000],"materials_modified":[]}[kind]);
