...
test.use({deviceScaleFactor:Number(process.env.PROBES_DPR||1)});
const view='canvas';
...
const cases={
 webgpu_lightprobe_cubecamera:{times:[0],parameters:[],at:0,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_materials_envmaps_hdr:{limits:[.005,1.8],times:[0,1],parameters:[[0,'Generated',0],[4,true,1],[0,'LDR',1],[1,0.5,0.5],[2,0.5,0.5],[3,1.5,1.5],[4,false,0]],restore:[[0,'HDR',2],[1,0,0],[2,0,0],[3,1,1]],at:1,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_loader_texture_ultrahdr:{limits:[.05,1.4],times:[0,1],parameters:[[1,0.5,0.5],[2,0.3,0.3],[3,1.5,1.5],[0,false,0]],restore:[[0,true,1],[1,1,1],[2,0,0],[3,1,1]],at:1,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgpu_materials_transmission:{limits:[.05,1.3],times:[0],parameters:[[1,0.5,0.5],[2,0.6,0.6],[4,0.3,0.3],[5,1.2,1.2],[6,2,2],[7,0.5,0.5],[9,0.5,0.5],[10,0.7,0.7],[0,0xff8844,0xff8844],[8,0x88ccff,0x88ccff]],restore:[[1,1,1],[2,1,1],[4,0,0],[5,1.5,1.5],[6,0.01,0.01],[7,1,1],[9,1,1],[10,1,1],[0,0xffffff,0xffffff],[8,0xffffff,0xffffff]],at:0,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgpu_performance:{times:[0],parameters:[[0,false,0]],restore:[[0,true,1]],at:0,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Probes and HDR official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/probes-hdr.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Probes and HDR GPU residency: ${kind}`,async({page},info)=>{
...
test('Probes and HDR resident geometry and official draw workload',async({page},info)=>{
...
if(count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/probes-hdr.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  const background={webgl_loader_texture_ultrahdr:36,webgpu_materials_transmission:5952}[kind];
...
for(const kind of [])test(`Probes and HDR on-demand scene stays idle: ${kind}`,async({page})=>{
