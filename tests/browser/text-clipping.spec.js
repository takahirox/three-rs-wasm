...
test.use({deviceScaleFactor:Number(process.env.TEXT_DPR||1)});
const view='canvas';
...
const cases={
 webgl_clipping_advanced:{times:[0,1,2.5,4],parameters:[[2,true,1],[3,true,1],[1,false,0],[0,false,0],[0,true,1],[1,true,1],[3,false,0]],at:4,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_geometry_extrude_splines:{rebuilds:true,limits:[.015,1.6],times:[0,3.3,7.1,15.6],parameters:[[7,true,1],[5,true,1],[6,true,1],[0,'PipeSpline',14],[4,false,0],[5,false,0],[1,6,6],[2,200,200],[3,6,6],[0,'HeartCurve',1]],at:7.1,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
...
// WebGL/WebGPU MSAA resolve bounds, documented in docs/text-clipping.md. The same
// scenes must also pass the ordinary threshold with MSAA disabled on both sides.
const msaaLimits={webgl_geometry_extrude_splines:[.07,1.8],webgl_geometry_text:[.01,.2],webgl_modifier_tessellation:[.08,2.6],webgl_custom_attributes_lines:[.14,1.6]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Text and clipping official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/text-clipping.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Text and clipping GPU residency: ${kind}`,async({page},info)=>{
...
test('Text and clipping resident geometry and official draw workload',async({page},info)=>{
...
if(count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/text-clipping.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const kind of [])test(`Text and clipping on-demand scene stays idle: ${kind}`,async({page})=>{
