...
const controls={furnace_test:[[0,true],[0,false]],geometry_convex:[],geometry_nurbs:[],geometry_text_shapes:[],geometry_text_stroke:[]};
const official=kind=>(kind==='furnace_test'?'webgpu_':'webgl_')+kind;
for(const samples of [1,4])for(const [kind,parameters] of Object.entries(controls))test(`Shapes official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  await page.setViewportSize({width:512,height:512});await page.goto(runtime==='reference'?`/reference/three-js/shapes.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  if(['geometry_convex','geometry_text_shapes','geometry_text_stroke'].includes(kind)){
  ...
  if(kind==='geometry_nurbs'){await page.mouse.move(256,256);await page.mouse.down();await page.mouse.move(296,256);await page.mouse.up();for(let i=0;i<180;i++)await capture(2.25,null,false);await capture(2.25);}
 ...
 writeFileSync(info.outputPath('comparison.json'),JSON.stringify(results,null,2));expect(errors).toEqual([]);for(const r of results){const limit=samples===4?({geometry_nurbs:.020,geometry_text_shapes:.020,geometry_text_stroke:.022}[kind]??.005):.005;expect(r.fraction,JSON.stringify(r)).toBeLessThanOrEqual(limit);expect(r.meanError,JSON.stringify(r)).toBeLessThanOrEqual(.6);}
});

for(const kind of Object.keys(controls)){const id=`${official(kind)}`;
 test(`Shapes GPU residency: ${kind}`,async({page},info)=>{
...
test('Shapes resident geometry and official draw workload',async({page},info)=>{
 ...
 for(const kind of Object.keys(controls)){
  const pair={kind};
  for(const runtime of ['reference','rust']){
   await page.goto(runtime==='reference'?`/reference/three-js/shapes.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  report.push(pair);const sort=a=>a.sort((a,b)=>a.count-b.count||a.instances-b.instances);
...
for(const kind of ['furnace_test','geometry_text_shapes','geometry_text_stroke'])test(`Shapes static scene stays idle: ${kind}`,async({page})=>{
