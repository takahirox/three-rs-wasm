...
test.use({deviceScaleFactor:Number(process.env.DRACO_DPR||1)});
const view='canvas';
...
const cases={
 webgl_ubo_arrays:{times:[0,1,2],parameters:[[0,50,50],[0,300,300]],restore:[[0,200,200]],at:1,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_loader_draco:{limits:[.03,.7],times:[0,1,3],parameters:[],at:1,antialias:true},
 webgl_animation_keyframes:{times:[0,1,2.5],parameters:[],at:1,antialias:true,settle:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_loader_gltf_variants:{times:[0],parameters:[[0,'beach',1],[0,'street',2]],restore:[[0,'midnight',0]],at:0,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgpu_morphtargets_face:{limits:[.005,.8],times:[0,1,2],parameters:[],at:1,antialias:true,settle:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
};
const official=kind=>kind;
// The original streams the 10,000 instance matrices and colors each frame.
const streams=['webgl_ubo_arrays'];
...
const msaaLimits={webgl_ubo_arrays:[.01,.15],webgl_animation_keyframes:[.1,2],webgl_loader_gltf_variants:[.01,.6]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Draco and variants official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/draco-variants.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Draco and variants GPU residency: ${kind}`,async({page},info)=>{
...
test('Draco and variants resident geometry and official draw workload',async({page},info)=>{
...
if(count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/draco-variants.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  const background={webgl_loader_gltf_variants:36}[kind];
...
for(const kind of [])test(`Draco and variants on-demand scene stays idle: ${kind}`,async({page})=>{
