...
test.use({deviceScaleFactor:Number(process.env.FLARES_DPR||1)});
const view='canvas';
...
const cases={
 webgl_raycaster_texture:{times:[0],parameters:[[0,1001,1],[1,1002,2],[2,0.3,0.3],[3,0.2,0.2],[4,1.5,1.5],[5,0.7,0.7],[6,0.5,0.5]],restore:[[0,1000,0],[1,1000,0],[2,0,0],[3,0,0],[4,1,1],[5,1,1],[6,0,0]],script:[['move',256,330,0],['move',90,330,0],['move',430,330,0]],at:0},
 webgl_texture3d_partialupdate:{times:[0,2,4,6],parameters:[[0,0.4,0.4],[1,0.5,0.5],[2,0.2,0.2],[3,50,50]],restore:[[0,0.25,0.25],[1,0.25,0.25],[2,0.1,0.1],[3,100,100]],at:6,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_materials_cubemap_render_to_mipmaps:{times:[0],parameters:[],at:0,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgpu_lensflares:{times:[0,1],parameters:[],script:[['move',300,220,1],['key','w',true,1.5],['key','w',false,2]],residency:[['move',640,360,1]],at:2},
 webgl_materials_car:{times:[0,1],parameters:[[0,0x00ff00,0x00ff00],[1,0x333333,0x333333],[2,0x88ccff,0x88ccff]],restore:[[0,0xff0000,0xff0000],[1,0xffffff,0xffffff],[2,0xffffff,0xffffff]],at:1,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
};
...
const msaaLimits={webgl_materials_car:[.05,.8]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Textures and flares official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/texture-flares.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Textures and flares GPU residency: ${kind}`,async({page},info)=>{
...
test('Textures and flares resident geometry and official draw workload',async({page},info)=>{
...
if(count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/texture-flares.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const kind of [])test(`Textures and flares on-demand scene stays idle: ${kind}`,async({page})=>{
