...
test.use({deviceScaleFactor:Number(process.env.TRACKBALL_DPR||1)});
const view='canvas';
...
const cases={
 webgl_loader_texture_hdr:{times:[0],parameters:[[0,1,1],[0,3.5,3.5],[0,.3,.3]],at:0},
 webgl_geometry_minecraft:{times:[0,.5],parameters:[],at:4.5,antialias:true,script:[['down',0,256,256,1],['move',330,280,1.5],['up',0,330,280,2],['key','KeyD',true,2.5],['key','KeyD',false,null],['key','KeyR',true,3],['key','KeyR',false,null],['down',2,256,256,3.5],['move',200,300,4],['up',2,200,300,4.5]]},
 misc_controls_trackball:{times:[0],parameters:[],at:4.5,antialias:true,script:[['drag',256,256,330,300,0,.5],['wait',1],['drag',256,256,300,200,2,1.5],['wheel',256,256,-400,2],['key','KeyS',true,null],['drag',256,300,256,200,0,2.5],['key','KeyS',false,null],['param',0,true,1,3],['drag',256,256,200,300,0,3.5],['wheel',256,256,300,4],['param',0,false,0,4.5]]},
 webgl_sprites:{times:[0,.5,1,2.5],parameters:[],at:2.5},
 webgl_lod:{times:[0,.5],parameters:[],at:4,antialias:true,script:[['move',400,300,1],['move',256,256,1.5],['down',0,256,256,2],['up',0,256,256,2.5],['key','KeyE',true,3],['key','KeyE',false,null],['key','ArrowUp',true,3.5],['key','ArrowUp',false,null],['down',2,300,200,4],['up',2,300,200,4]]},
};
...
// WebGL/WebGPU MSAA resolve bounds, documented in docs/trackball-sprites.md. The same
// scenes must also pass the ordinary threshold with MSAA disabled on both sides.
const msaaLimits={webgl_geometry_minecraft:[.0075,.15],misc_controls_trackball:[.015,.25],webgl_lod:[.2,7.5]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Trackball and sprites official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/trackball-sprites.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
 ...
 // The dense LOD wireframes must carry the original's total light in every state: per-sample
 // line coverage may differ with MSAA, the drawn levels and their shading may not.
 if(kind==='webgl_lod')for(const r of results){const mean=image=>{let sum=0;for(let p=0;p<image.data.length;p+=4)sum+=image.data[p]+image.data[p+1]+image.data[p+2];return sum/(image.data.length/4*3);};const a=mean(images.rust[r.state]),b=mean(images.reference[r.state]);r.meanLevel=[a,b];expect(Math.abs(a-b),JSON.stringify(r)).toBeLessThanOrEqual(Math.max(.1,.005*b));}
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Trackball and sprites GPU residency: ${kind}`,async({page},info)=>{
  ...
  for(const resize of [false,true]){
   ...
   const cycle=async()=>{
    ...
    for(const step of spec.script??[]){const time=await act(page,'rust',step);await frames(page,'rust',time??spec.at,1);}
...
test('Trackball and sprites resident geometry and official draw workload',async({page},info)=>{
...
if(a[0]!==4||count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/trackball-sprites.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
test('Trackball and sprites on-demand scene stays idle: webgl_loader_texture_hdr',async({page})=>{
 await page.goto('/web/gallery/example.html?id=webgl_loader_texture_hdr');await page.waitForFunction(()=>Number(document.querySelector('canvas')?.dataset.frames)>0);
