...
test.use({deviceScaleFactor:Number(process.env.REFRACTION_DPR||1)});
const view='canvas';
...
const cases={
 webgl_materials_cubemap_refraction:{limits:[.007,.6],times:[0,.5,1.5],parameters:[],at:1.5,antialias:true,script:[['move',100,100,2],['move',450,420,3],['wait',4.5]]},
 webgl_loader_ply:{limits:[.01,.6],times:[0,1,2.5,4,6],parameters:[],at:0,antialias:true},
 webgl_loader_kmz:{times:[0],parameters:[],at:0,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300],pan:[[256,256],[300,220]]},
 webgl_loader_collada:{times:[0,1,3,6],parameters:[],at:0,antialias:false},
 webgl_loader_texture_exr:{times:[0],parameters:[[0,.5,.5],[0,4,4],[0,2,2]],at:0,antialias:false},
};
...
// WebGL/WebGPU MSAA resolve bounds, documented in docs/refraction-loaders.md. The same
// scenes must also pass the ordinary threshold with MSAA disabled on both sides, except
// the refracted statues' fine environment detail and the PLY scene's flat facets and
// shadow edges (documented there).
const msaaLimits={webgl_materials_cubemap_refraction:[.03,1],webgl_loader_ply:[.03,.5],webgl_loader_kmz:[.03,.4]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Refraction and loaders official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/refraction-loaders.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Refraction and loaders GPU residency: ${kind}`,async({page},info)=>{
...
test('Refraction and loaders resident geometry and official draw workload',async({page},info)=>{
...
if(a[0]!==4||count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/refraction-loaders.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const kind of ['webgl_loader_kmz','webgl_loader_texture_exr'])test(`Refraction and loaders on-demand scene stays idle: ${kind}`,async({page})=>{
