...
test.use({deviceScaleFactor:Number(process.env.CONTROLS_DPR||1)});
const view='canvas';
// Per example: capture times, [control index, reference value, Rust value] at its time, input script.
const cases={
 misc_controls_orbit:{settle:true,times:[0],parameters:[],at:0,antialias:true,drag:[[256,256],[330,300]],pan:[[256,256],[300,230]],wheel:[256,256,-300]},
 misc_controls_map:{settle:true,times:[0],parameters:[[1,true,1]],at:0,antialias:true,drag:[[256,256],[320,300]],pan:[[256,256],[200,300]],wheel:[380,150,-400],late:[[0,true,1]],lateWheel:[120,380,-400]},
 webgl_camera:{times:[0,.5,1.3,2.7],parameters:[],at:2.7,antialias:true,keys:[['o',3.1],['p',3.5]]},
 webgl_custom_attributes:{times:[0,.25,.5,1,2],parameters:[],at:2},
 webgl_buffergeometry_drawrange:{times:[0,.2,.5,1],parameters:[[2,200,200,1.1],[3,true,1,1.2],[4,5,5,1.3],[5,800,800,1.4],[0,false,0,1.5],[0,true,1,1.6],[1,false,0,1.7],[1,true,1,1.8],[3,false,0,1.9]],at:2,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
};
const official=kind=>kind;
// WebGL/WebGPU MSAA resolve bounds, documented in docs/controls-attributes.md. The same
// scenes must also pass the ordinary threshold with MSAA disabled on both sides.
const msaaLimits={misc_controls_orbit:[.01,.15],misc_controls_map:[.0075,.15],webgl_camera:[.05,1.8],webgl_buffergeometry_drawrange:[.33,9.2]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Controls and attributes official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/controls-attributes.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  for(const [key,time] of spec.keys??[]){await page.keyboard.press(key);await capture(time);}
  ...
  for(const parameter of spec.late??[])await capture(t,parameter);
  if(spec.lateWheel)await wheel(spec.lateWheel);
 ...
 writeFileSync(info.outputPath('comparison.json'),JSON.stringify(results,null,2));expect(errors).toEqual([]);
  // The additive connection lines must carry the same total light as the original's in every
  // state: per-sample line coverage may differ with MSAA, the drawn lines and weights may not.
 if(kind==='webgl_buffergeometry_drawrange')for(const r of results){const mean=image=>{let sum=0;for(let p=0;p<image.data.length;p+=4)sum+=image.data[p]+image.data[p+1]+image.data[p+2];return sum/(image.data.length/4*3);};const a=mean(images.rust[r.state]),b=mean(images.reference[r.state]);r.meanLevel=[a,b];expect(Math.abs(a-b),JSON.stringify(r)).toBeLessThanOrEqual(Math.max(.1,.005*b));}
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Controls and attributes GPU residency: ${kind}`,async({page},info)=>{
  ...
  for(const resize of [false,true]){
   ...
   const cycle=async()=>{
    ...
    for(const [i,,v] of [...spec.parameters,...(spec.late??[])]){await page.evaluate(([i,v])=>app.tsl_parameter(i,v),[i,v]);await frames(page,'rust',spec.at,1);}
    for(const [key] of spec.keys??[]){await page.keyboard.press(key);await frames(page,'rust',spec.at,1);}
...
test('Controls and attributes resident geometry and official draw workload',async({page},info)=>{
...
// Fullscreen triangles (the port's per-view clear and present pass) are left out on both sides.
if(a[0]!==4||count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/controls-attributes.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  // Only the two examples whose originals stream CPU-computed attributes write any; neither
  // writes more than the original uploads.
  if(['webgl_custom_attributes','webgl_buffergeometry_drawrange'].includes(kind))expect.soft(pair.rust.attributeBytes,kind).toBeLessThanOrEqual(pair.reference.attributeBytes);
