...
test.use({deviceScaleFactor:Number(process.env.SELECTION_DPR||1)});
const view='canvas';
...
const cases={
 misc_animation_groups:{times:[0,.5,1,1.5,2.2,2.9,4],parameters:[],at:1.5,antialias:true},
 misc_animation_keys:{times:[0,.5,1,1.5,2.2,2.9,4],parameters:[],at:1.5,antialias:true},
 misc_controls_drag:{times:[0],parameters:[],at:0,antialias:true,script:[['drag',256,256,330,300,0,0],['drag',200,200,150,260,2,0],['key','Shift',true,null],['down',0,300,250,null],['up',0,300,250,0],['down',0,200,300,null],['up',0,200,300,0],['key','Shift',false,null],['drag',300,250,350,200,0,0]],residency:[['drag',256,256,330,300,0,0],['drag',200,200,150,260,2,0],['key','Shift',true,null],['down',0,300,250,null],['up',0,300,250,0],['down',0,300,250,null],['up',0,300,250,0],['key','Shift',false,null]]},
 misc_boxselection:{times:[0],parameters:[],at:0,antialias:true,script:[['down',0,150,150,null],['move',300,280,0],['up',0,350,330,0],['down',0,100,300,null],['up',0,100,300,0]]},
 webgpu_camera_array:{times:[0,1,2.5],parameters:[],at:2.5,limits:[.015,.4]},
};
...
// WebGL/WebGPU MSAA resolve bounds, documented in docs/selection-views.md. The same
// scenes must also pass the ordinary threshold with MSAA disabled on both sides.
const msaaLimits={misc_animation_groups:[.01,.2],misc_animation_keys:[.01,.2],misc_controls_drag:[.03,.7],misc_boxselection:[.03,.8]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Selection and views official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/selection-views.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Selection and views GPU residency: ${kind}`,async({page},info)=>{
...
test('Selection and views resident geometry and official draw workload',async({page},info)=>{
...
if(count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/selection-views.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const kind of ['misc_controls_drag'])test(`Selection and views on-demand scene stays idle: ${kind}`,async({page})=>{
