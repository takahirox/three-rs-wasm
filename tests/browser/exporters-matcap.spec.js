...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Exporters and matcap official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/exporters-matcap.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  await page.addStyleTag({content:'#notice,#settings,#info,#selectBox{display:none!important}'});
  const shots=[];
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Exporters and matcap GPU residency: ${kind}`,async({page},info)=>{
...
test('Exporters and matcap resident geometry and official draw workload',async({page},info)=>{
...
if(count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/exporters-matcap.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const kind of [])test(`Exporters and matcap on-demand scene stays idle: ${kind}`,async({page})=>{
...
for(const [kind,sequences] of Object.entries(exports))test(`Exporters and matcap exported files: ${kind}`,async({page},info)=>{
 test.setTimeout(120000);const files={};
 for(const runtime of ['reference','rust']){
  await page.setViewportSize({width:512,height:512});
  await page.goto(runtime!=='rust'?`/reference/three-js/exporters-matcap.html?id=${kind}`:`/web/gallery/example.html?id=${kind}&still=1`);
  ...
  for(const sequence of sequences){
   for(const i of sequence.slice(0,-1)){// The animation loop renders between clicks, updating world matrices.
    if(runtime!=='rust')await page.evaluate(async i=>{fixtureParameter(i,0);await renderFixture(0);},i);else{await page.evaluate(i=>app.tsl_parameter(i,1),i);await frames(page,'rust',0,1);}}
   const last=sequence.at(-1);
   const bytes=await page.evaluate(async({runtime,last})=>{
    if(runtime!=='rust'){const n=fixtureExports.length;fixtureParameter(last,0);while(fixtureExports.length===n)await new Promise(r=>requestAnimationFrame(r));return Array.from(new Uint8Array(await fixtureExports.at(-1).arrayBuffer()));}
    app.tsl_parameter(last,1);const file=app.gallery_take_export();return Array.from(file[1]);
   },{runtime,last});
   files[runtime].push(bytes);
  }
 }
 for(let k=0;k<sequences.length;k++){writeFileSync(info.outputPath(`${k}-rust.bin`),Buffer.from(files.rust[k]));writeFileSync(info.outputPath(`${k}-reference.bin`),Buffer.from(files.reference[k]));expect(files.rust[k].length,`${kind} ${k} size`).toBe(files.reference[k].length);expect(Buffer.from(files.rust[k]).equals(Buffer.from(files.reference[k])),`${kind} ${k} bytes`).toBe(true);}
});
