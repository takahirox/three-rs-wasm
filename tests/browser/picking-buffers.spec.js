...
test.use({deviceScaleFactor:Number(process.env.PICKING_DPR||1)});
const view='canvas';
...
const cases={
 webgl_loader_bvh:{limits:[.005,.7],times:[0,.4,1,1.8,2.6,3.5,4.4,5.3],parameters:[],at:2,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300],pan:[[256,256],[300,220]]},
...
const streams=['webgl_loader_bvh','webgl_framebuffer_texture'];
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Picking and buffers official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/picking-buffers.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  await page.addStyleTag({content:'#notice,#settings,#info{display:none!important}#selection>div,#values{visibility:hidden!important}'});
  const shots=[];
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Picking and buffers GPU residency: ${kind}`,async({page},info)=>{
...
test('Picking and buffers resident geometry and official draw workload',async({page},info)=>{
...
if(a[0]!==4||count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/picking-buffers.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  // The skeleton helper positions and the gosper colors are streamed by the original too.
...
test('Picking and buffers float readback values: webgl_read_float_buffer',async({page},info)=>{
 const report=[];
 for(const runtime of ['reference','rust']){
  await page.setViewportSize({width:512,height:512});await page.mouse.move(511,0);
  await page.goto(runtime!=='rust'?'/reference/three-js/picking-buffers.html?id=webgl_read_float_buffer':'/web/gallery/example.html?id=webgl_read_float_buffer&still=1');
  await page.waitForFunction(()=>{const c=document.querySelector('canvas');return c?.dataset.ready==='true'||Number(c?.dataset.frames)>0;});
  const values=[];
  for(const [x,y,t] of [[256,256,0],[100,100,.5],[400,300,1],[300,50,1.4],[20,500,2]]){
   await page.mouse.move(x,y);await frames(page,runtime,t,1);
   for(let k=0;k<3;k++){await page.waitForTimeout(50);await frames(page,runtime,t,1);}
   values.push((await page.locator('#values').innerHTML()).split('<br>').map(v=>Number(v.split(':')[1])));
  }
  report.push({runtime,values});
 }
 writeFileSync(info.outputPath('values.json'),JSON.stringify(report,null,2));
 const [reference,rust]=report.map(r=>r.values);
 for(let i=0;i<reference.length;i++)for(let c=0;c<3;c++)expect(Math.abs(rust[i][c]-reference[i][c]),JSON.stringify({i,reference:reference[i],rust:rust[i]})).toBeLessThanOrEqual(2e-3);
});
