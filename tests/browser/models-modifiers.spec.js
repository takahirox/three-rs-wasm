...
test.use({deviceScaleFactor:Number(process.env.MODELS_DPR||1)});
const view='canvas';
...
const cases={
 webgl_loader_mdd:{times:[0,.3,.8,1.5,2.2,2.9,3.4],parameters:[],at:3.4,antialias:true},
 webgl_modifier_edgesplit:{times:[0],parameters:[[0,true,1],[1,false,0],[1,true,1],[2,false,0],[2,true,1],[3,60,60],[4,false,0]],restore:[[0,false,0],[3,20,20],[4,true,1]],at:0,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_loader_3ds:{times:[0],parameters:[],at:3,antialias:true,script:[['drag',256,256,330,300,0,1],['drag',256,256,300,200,2,1.5],['wheel',256,256,-400,2],['key','KeyS',true,null],['drag',256,300,256,200,0,2.5],['key','KeyS',false,null],['wait',3]]},
 webgl_geometry_teapot:{times:[0],parameters:[[6,'wireframe',0],[6,'flat',1],[6,'smooth',2],[6,'textured',4],[6,'reflective',5],[0,30,9],[1,false,0],[4,true,1],[5,true,1],[3,false,0],[6,'glossy',3]],restore:[[0,15,7],[1,true,1],[4,false,0],[5,false,0],[3,true,1]],at:0,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_instancing_scatter:{times:[0,.5,1,2,3.5],parameters:[[0,800,800,3.6],[1,'weighted',1,3.7]],at:4,antialias:true},
};
...
// WebGL/WebGPU MSAA resolve bounds, documented in docs/models-modifiers.md. The same
// scenes must also pass the ordinary threshold with MSAA disabled on both sides.
const msaaLimits={webgl_modifier_edgesplit:[.0075,.25],webgl_loader_3ds:[.012,.3],webgl_geometry_teapot:[.085,5.5],webgl_instancing_scatter:[.045,.95]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Models and modifiers official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/models-modifiers.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Models and modifiers GPU residency: ${kind}`,async({page},info)=>{
  ...
  for(const resize of [false,true]){
   ...
   const cycle=async()=>{
    ...
    for(const [i,,v] of [...spec.parameters,...(spec.restore??[])]){await page.evaluate(([i,v])=>app.tsl_parameter(i,v),[i,v]);await frames(page,'rust',spec.at,1);}
...
test('Models and modifiers resident geometry and official draw workload',async({page},info)=>{
...
if(a[0]!==4||count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/models-modifiers.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
test('Models and modifiers on-demand scene stays idle: webgl_geometry_teapot',async({page})=>{
 await page.goto('/web/gallery/example.html?id=webgl_geometry_teapot');await page.waitForFunction(()=>Number(document.querySelector('canvas')?.dataset.frames)>0);
