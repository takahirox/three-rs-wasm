...
test.use({deviceScaleFactor:Number(process.env.SKY_DPR||1)});
const view='canvas';
...
const cases={
 webgpu_sky:{times:[0],parameters:[[4,30,30],[5,90,90],[5,-120,-120],[0,2,2],[1,1,1],[7,false,0],[8,0.8,0.8],[9,0.8,0.8],[10,0.9,0.9],[6,0.3,0.3],[2,0.02,0.02],[3,0.9,0.9]],restore:[[4,65,65],[5,0,0],[0,10,10],[1,3,3],[7,true,1],[8,0.4,0.4],[9,0.4,0.4],[10,0.5,0.5],[6,0.05,0.05],[2,0.005,0.005],[3,0.7,0.7]],at:0,drag:[[256,256],[330,300]]},
 // FirstPersonControls step by the timer's delta: capture times only move forward.
 webgpu_lights_sunlight:{rebuilds:true,times:[0,1,2],parameters:[[3,200,200,2],[4,40,40,2],[1,500,500,2],[2,1024,2,2]],at:6,antialias:true,script:[['key','w',true,null],['wait',3],['key','w',false,null],['wait',4],['down',0,300,256,null],['move',350,256,4.5],['up',0,350,256,5]]},
 misc_controls_pointerlock:{times:[0],parameters:[],at:0,antialias:true,script:[['lock',0],['key','w',true,null],['wait',1],['key','w',false,null],['wait',2],['look',120,40,2.5],['key',' ',true,null],['key',' ',false,2.6],['wait',3.5],['key','d',true,null],['wait',4],['key','d',false,5]]},
 // The video plays in real time: captures pause it and seek both pages to the same frame.
 webgpu_video_panorama:{times:[],parameters:[],at:0,antialias:true,script:[['video',1,0],['video',4.5,0],['drag',256,256,330,300,0,0],['drag',200,300,120,200,0,0]]},
 // updateSun() moves the sky through the PMREM scene; the first frame after it reflects
 // no sky, so captures take the settled frames (pick renders three more).
 webgpu_ocean:{rebuilds:true,pick:true,times:[0,1,2.5],parameters:[[0,10,10],[1,120,120],[2,0.3,0.3],[3,6,6],[4,3,3],[5,1,1],[6,0.5,0.5],[7,0.7,0.7],[8,0.8,0.8],[9,0.2,0.2]],at:1,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
};
...
// WebGL/WebGPU MSAA resolve bounds, documented in docs/sky-water.md. The same
// scenes must also pass the ordinary threshold with MSAA disabled on both sides.
const msaaLimits={webgpu_lights_sunlight:[.02,.4],misc_controls_pointerlock:[.02,.4],webgpu_video_panorama:[.02,.4]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Sky and water official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/sky-water.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
  ...
  await page.addStyleTag({content:'#notice,#settings,#info,#selectBox,#blocker{display:none!important}'});
  const shots=[];
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Sky and water GPU residency: ${kind}`,async({page},info)=>{
...
test('Sky and water resident geometry and official draw workload',async({page},info)=>{
...
if(count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/sky-water.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const kind of [])test(`Sky and water on-demand scene stays idle: ${kind}`,async({page})=>{
