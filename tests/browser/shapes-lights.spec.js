...
test.use({deviceScaleFactor:Number(process.env.SHAPES_DPR||1)});
const view='canvas';
...
const cases={
 webgl_materials_cubemap:{times:[0],parameters:[],at:0,antialias:true,drag:[[256,256],[330,300]]},
 webgl_loader_stl:{times:[0,1,2.5,4,6],parameters:[],at:0,antialias:true},
 webgl_geometry_extrude_shapes:{times:[0],parameters:[],at:0,antialias:true,script:[['drag',256,256,330,300,0,1],['drag',256,256,300,200,2,1.5],['wheel',256,256,-400,2],['wait',3]]},
 webgl_lights_spotlights:{times:[0,1,2.5,4.5,5,6.5,9,10.5],parameters:[],at:10.5,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_lights_hemisphere:{rebuilds:true,times:[0,.3,.7,1.2],parameters:[[0,true,0],[1,true,0],[0,true,1],[1,true,1]],at:1.2,antialias:true},
};
...
// WebGL/WebGPU MSAA resolve bounds, documented in docs/shapes-lights.md. The same
// scenes must also pass the ordinary threshold with MSAA disabled on both sides.
const msaaLimits={webgl_materials_cubemap:[.02,.35],webgl_loader_stl:[.01,.2],webgl_geometry_extrude_shapes:[.01,.2],webgl_lights_spotlights:[.025,.7],webgl_lights_hemisphere:[.01,.25]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Shapes and lights official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/shapes-lights.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Shapes and lights GPU residency: ${kind}`,async({page},info)=>{
...
test('Shapes and lights resident geometry and official draw workload',async({page},info)=>{
...
if(count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/shapes-lights.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
for(const kind of [])test(`Shapes and lights on-demand scene stays idle: ${kind}`,async({page})=>{
