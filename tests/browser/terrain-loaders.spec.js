...
test.use({deviceScaleFactor:Number(process.env.TERRAIN_DPR||1)});
const view='canvas';
...
const cases={
 webgl_geometry_terrain:{times:[0,.5],parameters:[],at:3,script:[['down',0,256,256,1],['move',330,280,1.5],['up',0,330,280,2],['key','KeyW',true,2.5],['key','KeyW',false,null],['key','KeyD',true,3],['key','KeyD',false,null]]},
 webgl_geometry_terrain_raycast:{times:[0],parameters:[],at:2.5,antialias:true,script:[['move',200,300,.5],['move',350,200,1],['drag',256,256,330,300,0,1.5],['move',256,256,2],['wheel',256,256,-300,2.5]]},
 webgl_loader_gcode:{times:[0],parameters:[[0,'test_m82',1],[0,'test_m83',2],[0,'benchy',0]],at:0,antialias:true,drag:[[256,256],[330,300]],wheel:[256,256,-300]},
 webgl_loader_vox:{times:[0],parameters:[],at:0,antialias:true,drag:[[256,256],[330,300]],pan:[[256,256],[300,230]],wheel:[256,256,-300]},
 webgl_loader_obj:{settle:true,times:[0],parameters:[],at:0,antialias:true,drag:[[256,256],[330,300]],pan:[[256,256],[300,230]],wheel:[256,256,-300]},
};
...
// WebGL/WebGPU MSAA resolve bounds, documented in docs/terrain-loaders.md. The same
// scenes must also pass the ordinary threshold with MSAA disabled on both sides.
const msaaLimits={webgl_geometry_terrain_raycast:[.01,.25],webgl_loader_gcode:[.13,5],webgl_loader_vox:[.025,.35]};
...
for(const [kind,spec] of Object.entries(cases))for(const samples of spec.antialias?[1,4]:[1])test(`Terrain and loaders official rendering: ${kind} samples=${samples}`,async({page},info)=>{
 ...
 for(const runtime of ['reference','rust']){
  ...
  await page.goto(runtime!=='rust'?`/reference/three-js/terrain-loaders.html?id=${official(kind)}&samples=${samples}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
 ...
 // The 1-pixel GCode lines must carry the original's total light in every state: per-sample
 // line coverage may differ with MSAA, the drawn segments and colors may not.
 if(kind==='webgl_loader_gcode')for(const r of results){const mean=image=>{let sum=0;for(let p=0;p<image.data.length;p+=4)sum+=image.data[p]+image.data[p+1]+image.data[p+2];return sum/(image.data.length/4*3);};const a=mean(images.rust[r.state]),b=mean(images.reference[r.state]);r.meanLevel=[a,b];expect(Math.abs(a-b),JSON.stringify(r)).toBeLessThanOrEqual(Math.max(.1,.005*b));}
...
for(const [kind,spec] of Object.entries(cases)){const id=official(kind);
 test(`Terrain and loaders GPU residency: ${kind}`,async({page},info)=>{
...
test('Terrain and loaders resident geometry and official draw workload',async({page},info)=>{
...
if(a[0]!==4||count>3)work.draws.push({count:a[0]===0?count*6:count,instances:1});return fn.apply(this,a);};}
 ...
 for(const [kind,spec] of Object.entries(cases)){
  ...
  for(const runtime of ['reference','rust']){
   ...
   await page.goto(runtime==='reference'?`/reference/three-js/terrain-loaders.html?id=${official(kind)}`:`/web/gallery/example.html?id=${official(kind)}&still=1`);
...
test('Terrain and loaders on-demand scene stays idle: webgl_loader_gcode',async({page})=>{
 await page.goto('/web/gallery/example.html?id=webgl_loader_gcode');await page.waitForFunction(()=>Number(document.querySelector('canvas')?.dataset.frames)>0);
